...
experiment = sacred.Experiment("Fetch -- HER")
...
def _run(env_name: str, progressive_noise: bool, small_goal: bool, small_goal_size: float, _config):
    ...
    params_parser = util.ConfigParser(her_td3.HerTD3Params)
    params = params_parser.parse(_config)
    agent = her_td3.HerTD3(functools.partial(make_env, env_name, progressive_noise, small_goal, small_goal_size), device, q1, q2, policy, params)
    train_fetch(experiment, agent, eval_env, progressive_noise, small_goal)


#if __name__ == '__main__':
    #experiment.add_config("iclr_experiments/experiments/hindsight/noisy_slide_config.json")
    #experiment.run()
